...
import { ScanLine, MousePointerClick, CalendarDays, Wallet, Clock, User } from "lucide-react";
...
export default function Dashboard() {
  ...
  const statCards = [
    ...
    { label: t('dash.today_revenue'), value: `${(stats?.todayRevenue || 0).toLocaleString()} UZS`, icon: Wallet, color: "text-emerald-400" },
  ];
          ...
          >
            <Card className="p-4 bg-card/50 backdrop-blur border-white/5 hover:border-white/10 transition-colors">
              <div className="flex items-center gap-3 mb-3">
                ...
                <h3 className="text-xs font-medium text-muted-foreground leading-tight">{stat.label}</h3>
              </div>
        ...
        <div className="space-y-3">
          {bookingsLoading ? (
            ...
            bookingsData.bookings.map((booking) => (
              <Card key={booking.id} className="p-4 bg-card border-white/5 flex items-center justify-between hover-lift">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><User className="w-3 h-3"/> {booking.serviceName || "Xizmat"}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-semibold text-primary mb-1">{booking.price.toLocaleString()}</div>
